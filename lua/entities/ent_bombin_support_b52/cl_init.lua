...
local TIER_OFFSETS = {
    [1] = { Vector(0, 0, 0) },
    [2] = { Vector(0,0,0), Vector(80,0,-5), Vector(-80,0,-5) },
    [3] = { Vector(0,0,5), Vector(80,0,-5), Vector(-80,0,-5), Vector(0,160,-8), Vector(0,-160,-8) },
}
...
local function SpawnBurstFX(ent, count, tier)
    ...
    for _ = 1, count do
        local wPos = LocalToWorld(Vector(math.Rand(-120,120), math.Rand(-200,80), math.Rand(-10,20)), Angle(0,0,0), pos, ang)
        BurstAt(wPos, tier)
    end

    if tier == 3 then
        for _, side in ipairs({ Vector(0,160,-8), Vector(0,-160,-8) }) do
...
net.Receive("bombin_plane_damage_tier_b52", function()
