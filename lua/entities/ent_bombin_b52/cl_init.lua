...
-- ============================================================
-- DAMAGE TIERS
-- Tier 1 (<66% HP): 2 fires (inner engines).
-- Tier 2 (<33%): 5 fires (all engine pods + fuselage).
-- Tier 3 (near-dead): 8 fires spread across full wingspan + fuselage
-- ============================================================
...
local function BurstAt( wPos, tier )
    ...
    ed:SetMagnitude( 1 )
    ed:SetRadius( tier * 25 )
    util.Effect( "Explosion", ed )

    local ed2 = EffectData()
    ed2:SetOrigin( wPos )
    ed2:SetNormal( Vector( 0, 0, 1 ) )
    ed2:SetScale( tier * 0.35 )
...
local function SpawnBurstFX( ent, count, tier )
    ...
    for _ = 1, count do
        local wPos = LocalToWorld(
            Vector( math.Rand( -100, 100 ), math.Rand( -240, 240 ), math.Rand( -15, 25 ) ),
            Angle( 0, 0, 0 ), pos, ang
        )
        BurstAt( wPos, tier )
    end
