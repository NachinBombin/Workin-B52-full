...
local function SpawnBurstFX(ent,count,tier)
    ...
    for _=1,count do local wPos=LocalToWorld(Vector(math.Rand(-100,100),math.Rand(-240,240),math.Rand(-15,25)),Angle(0,0,0),pos,ang) BurstAt(wPos,tier) end
    if tier==3 then
        for _,side in ipairs({Vector(0,230,-10),Vector(0,-230,-10)}) do
            local wPos=LocalToWorld(side,Angle(0,0,0),pos,ang)
            local ed=EffectData() ed:SetOrigin(wPos) ed:SetScale(0.9) ed:SetMagnitude(1) ed:SetRadius(40) util.Effect("Explosion",ed)
        end
    end
end
    ...
    for entIndex,state in pairs(PlaneStates) do
        ...
        if not IsValid(ent) then StopParticles(state) PlaneStates[entIndex]=nil
            ...
            if state.tier>0 then
                ...
                if ct>=state.nextBurst then SpawnBurstFX(ent,TIER_BURST_COUNT[state.tier] or 1,state.tier) state.nextBurst=ct+(TIER_BURST_DELAY[state.tier] or 4) end
            end
        end
    end
end)
