...
function ENT:Initialize()
    ...
    if self.CallDir:LengthSqr() <= 0.01 then self.CallDir = Vector(1,0,0) end
    self.CallDir.z = 0
    self.CallDir:Normalize()

    -- Sky height: try FindGround; if it fails or returns deeper than player, use player Z
    local ground = self:FindGround(self.CenterPos)
    local groundBase
    if ground ~= -1 and ground >= self.CenterPos.z - 1024 then
        groundBase = ground
    else
    ...
    self:SetMoveType(MOVETYPE_NONE)   -- anim entity, we move it manually in Think
...
function ENT:StartTumble()
    ...
    self.TumbleVelocity = Vector(fwd.x*spd, fwd.y*spd, -150)
...
function ENT:CrashExplode()
    ...
    E(Vector(0,0,100),5,5,500,"500lb_air")
...
function ENT:DestroyUAV()
    ...
    if self.EngineLoop then self.EngineLoop:ChangeVolume(0,0.3) timer.Simple(0.4,function() if self.EngineLoop then self.EngineLoop:Stop() end end) end
    self:StartTumble()
    timer.Simple(12,function() if IsValid(self) then self:CrashExplode() end end)
end

-- ============================================================
-- THINK  (all movement is here -- entity type is "anim")
-- ============================================================
function ENT:Think()
    if not self.DieTime or not self.SpawnTime then
        self:NextThink(CurTime()+0.05) return true
    end

    local ct = CurTime()
    local dt = FrameTime()         -- use FrameTime for anim entities
    ...
    newPos.z     = Lerp(0.12, pos.z, liveAlt)
    ...
    -- Derived velocity for weapon inheritance
    self.DerivedVelocity = (newPos - pos) / dt
...
function ENT:GetMuzzleWorldPos(localPoint) return self:LocalToWorld(localPoint) end
...
function ENT:PickNewWeapon(ct)
    self.CurrentWeapon   = (math.random(1,2)==1) and "s8_salvo" or "vikhr"
...
function ENT:UpdateS8Salvo(ct)
    ...
    if dir:LengthSqr()<1 then return end dir:Normalize()
    ...
    if IsValid(rPhys) then rPhys:AddVelocity(self.DerivedVelocity) end
    ...
    local ref=rocket timer.Simple(0.5,function() if IsValid(ref) and IsValid(self) then constraint.RemoveConstraints(ref,"NoCollide") end end)
end
...
function ENT:UpdateVikhr(ct)
    ...
    rocket.OnExplode=function(s,p,n)
        if oldE then oldE(s,p,n) end if s.IdleSound then s.IdleSound:Stop() end
...
function ENT:FindGround(centerPos)
    ...
    local filterList={self} local maxIter=0
...
function ENT:Debug(msg)
    ...
    print(full)
    for _,ply in ipairs(player.GetHumans()) do if IsValid(ply) then ply:PrintMessage(HUD_PRINTCONSOLE,full) end end
end
