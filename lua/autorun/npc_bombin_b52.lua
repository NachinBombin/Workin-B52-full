if SERVER then
    ...
    local function ThrowSupportFlare(npc, targetPos)
        ...
        local throwVel = toTarget * 400 + Vector(0, 0, 200)
        flare:Spawn()
        flare:Activate()
    ...
    local function FireBombinB52(npc, target)
    ...
    timer.Create("BombinB52_Think", 0.5, 0, function()
        ...
        for _, npc in ipairs(ents.GetAll()) do
            ...
            if FireBombinB52(npc, enemy) then
                npc.__bombinb52_lastCall = now
                BSP_Debug("B-52 call accepted targeting " .. tostring(enemy))
            end
        end
    end)
