if SERVER then
    ...
    util.AddNetworkString("BombinB52_FlareSpawned")
    ...
    local cv_life     = CreateConVar("npc_bombinb52_lifetime",  "120",  SHARED_FLAGS, "B-52 lifetime seconds")
    local cv_speed    = CreateConVar("npc_bombinb52_speed",     "260",  SHARED_FLAGS, "B-52 forward speed HU/s")
    local cv_radius   = CreateConVar("npc_bombinb52_radius",    "4200", SHARED_FLAGS, "Orbit radius HU")
    local cv_height   = CreateConVar("npc_bombinb52_height",    "7000", SHARED_FLAGS, "Altitude above ground HU")
    ...
    local function ThrowSupportFlare(npc, targetPos)
        ...
        net.Start("BombinB52_FlareSpawned")
    ...
    local function SpawnB52AtPos(centerPos)
        if not scripted_ents.GetStored("ent_bombin_b52") then
            BSP_Debug("ent_bombin_b52 not registered")
            return false
        end

        local b52 = ents.Create("ent_bombin_b52")
        if not IsValid(b52) then BSP_Debug("ents.Create returned invalid entity") return false end
        ...
        if not IsValid(b52) then BSP_Debug("Entity invalid after Spawn()") return false end

        BSP_Debug("B-52 spawned at " .. tostring(centerPos) .. " dir " .. tostring(randomDir))
        return true
    end

    local function FireB52(npc, target)
    ...
    net.Receive("BombinB52_ManualSpawn", function(len, ply)
        ...
        if not scripted_ents.GetStored("ent_bombin_b52") then
            ply:PrintMessage(HUD_PRINTCENTER, "[Bombin B-52] Entity not registered!") return
        end

        local b52 = ents.Create("ent_bombin_b52")
        if not IsValid(b52) then
            ply:PrintMessage(HUD_PRINTCENTER, "[Bombin B-52] Spawn failed!") return
        end
    ...
    -- ── NPC poll timer ───────────────────────────────────────────
    timer.Create("BombinB52_Think", 0.5, 0, function()
...
if CLIENT then
    ...
    net.Receive("BombinB52_FlareSpawned", function()
        local flare = net.ReadEntity()
        if IsValid(flare) then activeFlares[flare:EntIndex()] = flare end
    end)

    hook.Add("Think", "BombinB52_FlareLight", function()
